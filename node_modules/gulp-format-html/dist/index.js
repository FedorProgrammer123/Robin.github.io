...
var formatHTML = (options = {}) => {
  ...
  return _through22.default.obj((file, enc, next) => {
    ...
    const beautify = (buf, _, cb) => {
      try {
        const contents = _buffer.Buffer.from(beautifyHtml(_nullishCoalesce((buf == null ? void 0 : buf.toString()), () => ( "")), options));
        if (next === cb) {
    ...
    if (file.isStream()) {
      ...
      if (options.verbose) {
        logger.info(`${_logger.c.yellow(PLUGIN_NAME)}: ${_logger.c.green(_path.relative.call(void 0, rootDir, file.path))}`);
      }
